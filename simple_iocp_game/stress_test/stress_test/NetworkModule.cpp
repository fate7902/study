...
void ProcessPacket(int ci, unsigned char packet[])
{
	...
	case SC_TRANS: 
	{
		SC_MOVE_PROTOCOL* move_packet = reinterpret_cast<SC_MOVE_PROTOCOL*>(packet);
	...
	case SC_LOGIN:
	{
		...
		active_clients++;
		SC_LOGIN_PROTOCOL* login_packet = reinterpret_cast<SC_LOGIN_PROTOCOL*>(packet);
		int my_id = ci;
...
void Test_Thread()
{
	while (true) {
		...
		for (int i = 0; i < num_connections; ++i) {
			...
			CS_MOVE_PROTOCOL my_packet;
			my_packet.size = sizeof(my_packet);
			my_packet.type = CS_MOVE;
			switch (rand() % 4) {
			case 0: my_packet.move_type = MOVE_TYPE::LEFT; break;
			case 1: my_packet.move_type = MOVE_TYPE::RIGHT; break;
			case 2: my_packet.move_type = MOVE_TYPE::UP; break;
			case 3: my_packet.move_type = MOVE_TYPE::DOWN; break;
			}
