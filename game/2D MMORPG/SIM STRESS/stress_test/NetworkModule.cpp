...
void ProcessPacket(int ci, unsigned char packet[])
{
	...
	case SC_MOVE_ALLOW: 
	{
		...
		if (move_packet->key < MAX_CLIENTS) {
			...
			if (ci == my_id) {
				if (0 != move_packet->clientTime) {
					auto d_ms = duration_cast<milliseconds>(high_resolution_clock::now().time_since_epoch()).count() - move_packet->clientTime;
	...
	case SC_ADDOBJECT_ALLOW: break;
	case SC_DELETEOBJECT_ALLOW: break;
	case SC_LOGIN_ALLOW:
	{
...
void Adjust_Number_Of_Client()
{
	...
	//sprintf_s(l_packet.name, "%d번", temp);
...
void Test_Thread()
{
	while (true) {
		...
		for (int i = 0; i < num_connections; ++i) {
			...
			CS_MOVE_REQUEST_PACKET my_packet;
			my_packet.size = sizeof(my_packet);
			my_packet.type = CS_MOVE_REQUEST;
			switch (rand() % 4) {
			case 0: my_packet.moveType = static_cast<int>(MOVETYPE::DOWN); break;
			case 1: my_packet.moveType = static_cast<int>(MOVETYPE::RIGHT); break;
			case 2: my_packet.moveType = static_cast<int>(MOVETYPE::UP); break;
			case 3: my_packet.moveType = static_cast<int>(MOVETYPE::DOWN); break;
			}
			my_packet.clientTime = static_cast<unsigned>(duration_cast<milliseconds>(high_resolution_clock::now().time_since_epoch()).count());
